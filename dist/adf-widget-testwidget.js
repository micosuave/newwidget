  ...
  .controller('CKEWidgetCtrl', ["$scope", "config", "ckdefault", "ckmin", "Collection", "$controller", "$rootScope","ckclip","ckreport","$ACTIVEROAR","$stateParams","$sce","$compile",
        function($scope, config, ckdefault, ckmin, Collection, $controller, $rootScope, ckclip, ckreport, $ACTIVEROAR, $stateParams, $sce, $compile) {
            ...
            draft.$loaded().then(function(d){
                ...
                $scope.sourcedoc = $sce.trustAsHtml($compile(angular.element(head + d.content)));
            });
...
angular.module("adf.widget.testwidget").run(["$templateCache", function($templateCache) {$templateCache.put("{widgetsPath}/testwidget/src/ckeditor.html","<div class=card style=\"margin: -0.1rem;padding: 0.1rem;text-align: left;overflow: visibile; height: 60rem;border: 0rem solid #110000;box-shadow:0 0 0 transparent;\" ng-if=pj.editable><div class=card-header ng-show=draft.isnotRoot><h3 class=card-title>{{draft.title}}</h3></div><div id={{config.id}} ckeditor=config.editor ng-model=draft.content ng-model-options=\"{ updateOn: \'default blur\', debounce: {\'default\': 30000, \'blur\': 0} }\" class=\"card card-block\" contenteditable=true style=width:100%;height:100%;font-size:12px;color:#444;></div></div><iframe id=draftdocument class={{draft.renderClass}} ng-if=!pj.editable srcdoc=\"{{sourcedoc | trustAsHTML}}\" style=width:100%;></iframe><textarea ng-model=inputhtml class=\"card card-dark\"></textarea><iframe srcdoc=\"{{inputhtml | trustAsHTML}}\" width=100%></iframe>");
