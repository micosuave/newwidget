...
$templateCache.put("{widgetsPath}/testwidget/src/ckeditor.html","<div class=card style=\"margin: -0.5rem;padding: 0rem;text-align: left;overflow: scroll; height: 50rem;border: 0.1rem solid #110000;\" ng-if=pj.editable><div class=card-header ng-hide=draft.isRoot><h3 class=card-title>{{draft.title}}</h3></div><div id=iframeElement ckeditor=config.editor ng-model=draft.content ng-model-options=\"{ updateOn: \'default blur\', debounce: {\'default\': 1000, \'blur\': 0} }\" class=\"card card-block card-rounded\" contenteditable=true style=width:100%;height:100%;font-size:14px;></div></div><div id=draftdocument class={{draft.renderClass}} ng-if=!pj.editable><ng-annotate-text text=draft.content></ng-annotate-text><div ng-repeat=\"node in draft.roarlist\" node=\"{{node.id || node}}\"><h2>{{node.title}}</h2><ng-annotate-text text=node.content></ng-annotate-text><div ng-repeat=\"node in node.roarlist\" node=\"{{node.id || node}}\"><h3>{{node.title}}</h3><ng-annotate-text text=node.content></ng-annotate-text><div ng-repeat=\"node in node.roarlist\" node=\"{{node.id || node}}\"><h4>{{node.title}}</h4><ng-annotate-text text=node.content></ng-annotate-text><div ng-repeat=\"node in node.roarlist\" node=\"{{node.id || node}}\"><h5>{{node.title}}</h5><ng-annotate-text text=node.content></ng-annotate-text><div ng-repeat=\"node in node.roarlist\" node=\"{{node.id || node}}\"><h6>{{node.title}}</h6><ng-annotate-text text=node.content></ng-annotate-text><div ng-repeat=\"node in node.roarlist\" node=\"{{node.id || node}}\"><label>{{node.title}}</label><ng-annotate-text text=node.content></ng-annotate-text></div></div></div></div></div></div></div>");
