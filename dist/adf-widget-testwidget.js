...
$templateCache.put("{widgetsPath}/testwidget/src/edit.html","<form role=form><div class=form-group><label for=draftid>Tree Root</label><ol><li class=\"card fa fa-file\" ng-repeat=\"node in tree.roarlist\">&nbsp;<a ng-click=\"config.id = node.id\" node={{node}}>{{node.title}}</a><div ng-include=\"\'quicklinkid\'\"></div></li></ol></div></form><script type=text/ng-template id=quicklinkid><ol> <li class=\"card fa fa-file\" ng-repeat=\"node in node.roarlist\" >&nbsp;<a ng-click=\"config.id = node.id\" node=\"{{node}}\">{{node.title}}</a> <div ng-include=\"\'quicklinkid\'\"></div> </li> </ol></script>");
$templateCache.put("{widgetsPath}/testwidget/src/editckeditor.html","<form role=form><div class=form-group><label for=draftid>Style Configuration</label><select ng-model=config.styleClass ng-options=\"class.value as class.label for class in ROARCLASSES\" class=form-control id=styleclass placeholder=\"Select style...\"><option label=default></option></select><input type=text ng-model=config.custom placeholder=\"custom style class(es)\" style=padding:0.8rem></div><div class=\"form-group col-sm-6\"><label for=renderframe>Render in iframe</label> <input type=checkbox ng-model=config.iframe></div><div class=\"form-group col-sm-6\"><label for=renderversionhistory>Version History</label> <input type=checkbox ng-model=config.versionhistory></div><div class=form-group><label for=draftid>Tree Root</label><ol><li class=\"card fa fa-file\" ng-repeat=\"node in tree.roarlist\">&nbsp;<a ng-click=\"config.id = node.id\" node={{node}}>{{node.title}}</a><div ng-include=\"\'quicklinkid\'\"></div></li></ol></div><script type=text/ng-template id=quicklinkid><ol> <li class=\"card fa fa-file\" ng-repeat=\"node in node.roarlist\" >&nbsp;<a ng-click=\"config.id = node.id\" node=\"{{node}}\">{{node.title}}</a> <div ng-include=\"\'quicklinkid\'\"></div> </li> </ol></script></form>");
