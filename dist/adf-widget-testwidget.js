...
angular.module("adf.widget.testwidget").run(["$templateCache", function($templateCache) {$templateCache.put("{widgetsPath}/testwidget/src/ckeditor.html","<div class=card style=\"margin: -0.1rem;padding: 0.1rem;text-align: left;overflow: scroll; height: 60rem;border: 0rem solid #110000;box-shadow:0 0 0 transparent;\" ng-if=config.showeditor><div class=card-header ng-show=draft.isnotRoot><h3 class=card-title>{{draft.title}}</h3></div><div id={{config.id}} ckeditor=config.editor ng-model=draft.content ng-model-options=\"{ updateOn: \'default blur\', debounce: {\'default\': 60000, \'blur\': 0} }\" class=\"card card-block\" style=width:100%;min-height:500px;font-size:12px;color:#444;></div></div><ng-annotate-text text=draft.content id=draftdocument class=\"card card-block {{draft.renderClass}}\" ng-if=!config.showeditor ng-attr-srcdoc={{draft.content}} style=width:100%;min-height:500px;></ng-annotate-text><iframe id=draftdocument2 class=\"card card-block {{draft.renderClass}}\" ng-if=!config.showeditor ng-attr-srcdoc={{draft.content}} style=width:100%;min-height:500px;></iframe><button ng-click=\"config.showeditor = !config.showeditor\" class=\"pull-right btn btn-default btn-sm fa\" ng-class=\"{\'fa-edit text-info\':!config.showeditor,\'fa-save text-success\':config.showeditor}\" style=position:absolute;top:0;right:-5px;border-radius:50%;></button>");
